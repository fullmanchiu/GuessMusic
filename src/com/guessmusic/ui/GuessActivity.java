...
public class GuessActivity extends Activity implements iWordButtonClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mDiscBarInAnim.setAnimationListener(new AnimationListener() {
			...
			public void onAnimationEnd(Animation animation) {
				...
				// 播放音乐
				MyPlayer.playSong(GuessActivity.this,
	...
	protected void onPause() {
		...
		// 暂停音乐
		MyPlayer.stopSong(GuessActivity.this);
		super.onPause();
	...
	private ArrayList<WordButton> initSelectWord() {
		...
		for (int i = 0; i < mCurrentSong.getSongNameLenth(); i++) {
			View view = Tools.getView(GuessActivity.this,
			...
			btnWord.mViewButton.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					// 播放音效
					MyPlayer.playTone(GuessActivity.this,
	...
	private void handlePassEvent() {
		...
		// 金币音效
		MyPlayer.playTone(GuessActivity.this, MyPlayer.INDEX_TONE_COIN);
		...
		btnPass.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// 播放音效
				MyPlayer.playTone(GuessActivity.this, MyPlayer.INDEX_TONE_ENTER);
				if (judegIsPassed()) {
					// 进入通关界面
					Tools.startIntent(GuessActivity.this, AllPassView.class);
				} else {
					// 开始下一关
	...
	private void showConfirmDialog(int id) {
		...
		case ID_DIALOG_DELET_WORD:
			Tools.showDialog(GuessActivity.this, "确认花掉" + getDeletWordCoin()
					+ "个金币去掉一个错误答案", mBtnConfirmDeletWordListener);
			break;
		case ID_DIALOG_TIP_WORD:
			Tools.showDialog(GuessActivity.this, "确认花掉" + getTipWordCoin()
					+ "个金币获得一个文字提示", mBtnConfirmTipWordListener);
			break;
		case ID_DIALOG_LACK_COINS_WORD:
			Tools.showDialog(GuessActivity.this, "金币不足，去商店补充？",
